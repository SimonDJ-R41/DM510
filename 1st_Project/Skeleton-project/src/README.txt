...
Making a 'newNode' that stores the given element in the data, then,
if there's still a next element, the list will point to its next, until end of the list (aka: its 'next' would point at NULL).
...
that way one could simply go to the end of the list and add it without iterating.
